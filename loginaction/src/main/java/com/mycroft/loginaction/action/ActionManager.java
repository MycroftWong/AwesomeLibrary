...
import com.mycroft.loginaction.annotation.Interceptor;
import com.mycroft.loginaction.exp.ValidException;
...
/**
 * @author mycroft
 * @Deprecated Created by jinyabo on 8/12/2017.
 */
public class ActionManager {
    ...
    private Stack<CallUnit> delaysActions = new Stack<>();

    /**
     * 根据条件判断，是否要执行一个action
     *
     * @param callUnit call unit
     */
    public void postCallUnit(CallUnit callUnit) {
        ...
        callUnit.check();
        //如果全部满足，则直接跳转目标方法
        Queue<Valid> queue = callUnit.getValidQueue();
        if (queue.isEmpty()) {
